...
BSSlocate::BSSlocate(const mat &sig, const double &dist, const int &freq)
{
  ...
  grid =
      linspace_fixed_step<>(-d / c, d / c, 2 * d / (c * (tauGridSize - 1)));
  if (grid.size() > 180)
...
vec BSSlocate::getSpec()
{
  ...
  for (int i = 0; i < vspec.length(); i++) {
    mspec.ins(i,
              sum(vspec[i],
                  1)); // alternative solution mspec.ins(i,max(vspec[i],1));
  }

  return mspec;
}

vec BSSlocate::getCC() { return (crossCorr / max(crossCorr)); }
...
Vec< mat > BSSlocate::nonLinSpec(const Vec< cmat > &X, const double &alpha,
                                 const vec &tau_grid)
{
  /*
  ...
  for ind = 1:ngrid,
      ...
      spec(:,:,ind) = ones(nbin,nfram) -
  tanh(alpha*real(sqrt(2-2*real(P.*EXP))));
  end
  ...
  for (int ind = 0; ind < ngrid; ind++) {
    EXP = repmat(itpp::exp(f * (-2) * std::complex< double >(0., 1.) * pi *
                           tau_grid(ind)),
                 1, nfram);
    u = elem_mult(P, EXP);
    spec[ind] = real(
        u); // alternative solution:spec[ind]=1-tanh(alpha*fastCSqrt(2-2*u));
  }
